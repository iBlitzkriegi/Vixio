...
public class EffReply extends Effect{
    ...
    protected void execute(Event e) {
        if(e instanceof EvntGuildMessageReceive) {JDA jda = ((EvntGuildMessageReceive)e).getJDA();
            TextChannel chnl = jda.getTextChannelById(((EvntGuildMessageReceive) e).getEvntChannel().getId());
            chnl.sendMessage(message.getSingle(e));
        }else if(e instanceof EvntPrivateMessageReceive){
            JDA jda = ((EvntPrivateMessageReceive)e).getJDA();
            jda.getPrivateChannelById(((EvntPrivateMessageReceive)e).getEvntChannel().getId()).sendMessage(message.getSingle(e));
        }else{
