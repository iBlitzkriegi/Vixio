 ...
 */
@ExprAnnotation.Expression(returntype = String.class, type = ExpressionType.SIMPLE, syntax = "[discord] [online] status of %string%")
    ...
    private String getAvatar(Event e) {
        for (Map.Entry<String, JDA> u : bots.entrySet()) {
            if(u.getValue().getUserById(vID.getSingle(e))!=null){
                if(!u.getValue().getUserById(vID.getSingle(e)).getOnlineStatus().name().equalsIgnoreCase("UNKNOWN")) {
                    return u.getValue().getUserById(vID.getSingle(e)).getOnlineStatus().name();
                }else{
