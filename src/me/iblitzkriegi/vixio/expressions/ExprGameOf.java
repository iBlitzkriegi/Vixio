 ...
 */
@ExprAnnotation.Expression(returntype = String.class, type = ExpressionType.SIMPLE, syntax = "%string%['s] current game")
    ...
    private String getGame(Event e) {
        for (Map.Entry<String, JDA> u : bots.entrySet()) {
            if(u.getValue().getUserById(vID.getSingle(e))!=null){
                if(u.getValue().getUserById(vID.getSingle(e)).getCurrentGame().getName()!=null) {
                    return u.getValue().getUserById(vID.getSingle(e)).getCurrentGame().getName();
                }else{
                    return "null";
                }
            }
